...
function BitButtonWidget({ variable, plcStateValue, opts, onWrite, isStale }) {
  ...
  // Valor exibido: se houver escrita pendente, usa o valor otimista; senão usa SEMPRE o valor real do CLP
  const displayActive = isPending && pendingVal !== null ? pendingVal : Boolean(plcStateValue);
  ...
  const handleClick = async () => {
    let newVal;
    if      (mode === 'toggle') newVal = !displayActive; // Inverte estado exibido
    else if (mode === 'set')    newVal = true;           // Força ligado
    else if (mode === 'reset')  newVal = false;          // Força desligado
    else return;

    setPendingVal(newVal);   // Mostra otimista
    setIsPending(true);

    await onWrite(variable, newVal);

    // Retorna ao estado do CLP após 1.5s (tempo para o poll capturar)
    setTimeout(() => {
      setIsPending(false);
      setPendingVal(null);
    }, 1500);
  ...
  // Pulsador NA (0 → 1 enquanto pressionado) / NF (1 → 0 enquanto pressionado)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', width: '100%', padding: '0.5rem' }}>
      <button
        ...
        style={{
          ...
          background: displayActive ? `${colorOn}25` : `${colorOff}15`,
          color: activeColor, fontWeight: 'bold', fontSize: '1rem',
          cursor: isPending ? 'wait' : 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem',
          boxShadow: displayActive ? `0 0 15px ${colorOn}40` : 'none',
...
function BitSwitchWidget({ variable, plcStateValue, opts, onWrite }) {
  ...
  // Valor exibido: se houver escrita pendente, usa pendingVal; senão usa SEMPRE o valor real do CLP
  const displayActive = isPending && pendingVal !== null ? pendingVal : Boolean(plcStateValue);
  ...
  const handleClick = async () => {
    ...
    let newVal;
    if      (mode === 'toggle') newVal = !displayActive;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
      <div
        ...
        style={{
          ...
          border: `2px solid ${activeColor}`,
          color: activeColor,
          background: displayActive ? `${activeColor}15` : 'var(--bg-subcard)',
          display: 'flex', alignItems: 'center', gap: '0.5rem',
          boxShadow: displayActive ? `0 0 15px ${activeColor}40` : 'none',
          cursor: cursorStyle,
      ...
      >
        <div style={{
          ...
          boxShadow: displayActive ? `0 0 10px ${activeColor}` : 'none',
          transition: 'all 0.15s ease'
        }} />
        {activeText}
              ...
              {variables.map((v, index) => {
                ...
                return (
                  <div key={'var-' + v.id.toString()} className="card" style={{ width: '100%', height: (['graph', 'timeseries', 'comparative_analysis', 'scatter'].includes(v.widget_type)) ? '280px' : undefined, minHeight: '170px', display: 'flex', flexDirection: 'column', padding: '1rem', borderTop: `4px solid ${v.color || 'var(--color-primary)'}` }}>
                    ...
                    {isStaleOverAWeek && v.widget_type !== 'header' ? (
                      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: '1.05rem', fontWeight: 500, userSelect: 'none' }}>
                        Sem dados
                      </div>
                    ) : (
                      ...
                      {(v.widget_type === 'input' || v.widget_type === 'write_button') && (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '0 0.5rem', width: '100%' }}>
                          <div style={{ fontSize: '1.8rem', fontWeight: 'bold', color: v.color || 'var(--text-primary)', marginBottom: '0.4rem' }}>
                            {displayVal}<span style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginLeft: '4px' }}>{v.unit}</span>
                          </div>
                      ...
                      <div style={{ width: '100%', height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.2rem', padding: '0.2rem 0.4rem' }}>
                        {items.map((item, idx) => {
                          ...
                          if (targetV && plcState && plcState[targetV.name] !== undefined) {
                            ...
                            if (targetV.type === 'boolean' || targetV.modbus_type === 'coil' || targetV.modbus_type === 'discrete') {
                              ...
                              isAnalog = true;
                              const numVal = typeof rawTargetVal === 'number' ? rawTargetVal : (parseFloat(rawTargetVal) || 0);
                              displayValStr = `${numVal.toFixed(targetV.decimals || 0)} ${targetV.unit || ''}`.trim();
                              isBitActive = numVal > 0;
                            }
                          }

                          const targetStaleMs = targetV ? (lastReadTimes[targetV.id] || lastReadTimes[targetV.name] || lastReadTimes[targetV.display_name]) : null;
                          const isTargetStale = targetV ? (!targetStaleMs || (Date.now() - targetStaleMs) >= 604800000) : false;
                          const activeLabel = isTargetStale ? 'Sem dados' : (isAnalog ? displayValStr : (isBitActive ? (item.label_on || 'LIGADO') : (item.label_off || 'DESLIGADO')));
                          const activeColor = isTargetStale ? 'var(--text-secondary)' : (isBitActive ? (item.color_on || v.color || '#10b981') : (item.color_off || 'var(--text-secondary)'));
                          const bgPill = isBitActive ? `${activeColor}25` : 'rgba(255, 255, 255, 0.06)';
                          const borderPill = isBitActive ? `1px solid ${activeColor}` : '1px solid rgba(255, 255, 255, 0.08)';
                        ...
                        {items.length === 0 && (
                          <div style={{ textAlign: 'center', padding: '1rem', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
                            Nenhuma variável configurada nesta lista.
                          </div>
                        )}
                      </div>
                    );
